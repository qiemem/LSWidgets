...
class LSArgumentSelector(changeCallback: ()=>Unit, ws: GUIWorkspace, maxTextLength: Option[Int]) extends JPanel {
  ...
  private var arguments = ListMap.empty[String, XWComboBox]
  ...
  private def selectedArgumentsLogoList(args: ListMap[String, XWComboBox]): LogoList =
    args.map {
      case (name: String, comboBox: XWComboBox) => LogoList(name, comboBox.selectedItem)
    }.toSeq.toLogo

  private def availableArgumentsLogoList(args: ListMap[String, XWComboBox]): LogoList =
    args.map {
      case (name: String, comboBox: XWComboBox) => LogoList(name, comboBox.items.toLogo)
    }.toSeq.toLogo

  private def setSelectedArguments(args: ListMap[String, XWComboBox], list: LogoList): Unit = list.foreach {
    case arg: LogoList =>
      val name = arg.get(0).asInstanceOf[String]
  ...
  private def assembleArguments(argumentList: LogoList): ListMap[String, XWComboBox] = {
  ...
  private def layoutArgumentPanel[T <: ExtraWidget, S](arguments: ListMap[String, XWComboBox]) = {
    removeAll()
